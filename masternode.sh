...
function create_sentinel() {
	...
	if [ ! -d ${sent_dir} ]; then
		cd ${data_dir}                                               &>> ${logfile}
		git clone https://github.com/fxtc/sentinel.git sentinel  	&>> ${logfile}
		cd sentinel                                                 &>> ${logfile}
	else
	...
	then
	    if [ ! -f "${sent_dir}/sentinel.conf" ]; then
             ...
             echo "network=testnet">> ${sent_dir}/sentinel.conf
             echo "db_name=${sent_dir}/database/sentinel.db">> ${sent_dir}/sentinel.conf
             echo "db_driver=sqlite">> ${sent_dir}/sentinel.conf
		     echo "dash_conf=${conf_dir}/${name}.conf"> ${sent_dir}/test/test_sentinel.conf
             echo "network=testnet">> ${sent_dir}/test/test_sentinel.conf
             echo "db_name=${sent_dir}/database/sentinel.db">> ${sent_dir}/test/test_sentinel.conf
             echo "db_driver=sqlite">> ${sent_dir}/test/test_sentinel.conf			 
        fi	
	else
		if [ ! -f "${sent_dir}/sentinel.conf" ]; then
	         echo "* Creating sentinel configuration for ${name} masternode on MAINNET" &>> ${logfile}    
		     echo "dash_conf=${conf_dir}/${name}.conf" > ${sent_dir}/sentinel.conf
             echo "network=mainnet">> ${sent_dir}/sentinel.conf
             echo "db_name=${sent_dir}/database/sentinel.db">> ${sent_dir}/sentinel.conf
             echo "db_driver=sqlite">> ${sent_dir}/sentinel.conf
		     echo "dash_conf=${conf_dir}/${name}.conf"> ${sent_dir}/test/test_sentinel.conf
             echo "network=mainnet">> ${sent_dir}/test/test_sentinel.conf
             echo "db_name=${sent_dir}/database/sentinel.db">> ${sent_dir}/test/test_sentinel.conf
             echo "db_driver=sqlite">> ${sent_dir}/test/test_sentinel.conf				 
        fi
	fi
	SENTINEL_CONFIG=${sent_dir}/sentinel.conf
	sudo -u ${user} -- bash -c "cd ${sent_dir} && venv/bin/python bin/sentinel.py" &>> ${logfile}
	sudo -u ${user} -- bash -c '"* * * * * export SENTINEL_CONFIG=${sent_dir}/sentinel.conf; cd ${sent_dir} && venv/bin/python bin/sentinel.py 2>&1 >> /var/log/sentinel/sentinel-cron.log" | crontab -' &>> ${logfile}
...
function configure_firewall() {
	...
	if [[ "$testnet" -eq 1  ]];	then
		ufw allow ${tport}/tcp     	&>> ${logfile}
		ufw allow ${trpcport}/tcp	&>> ${logfile}
