...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                // Veles: dash.conf corrected to veles.conf in all the file.
                "  vote-conf          - Vote on a governance object by masternode configured in veles.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("veles.conf", statusObj));
