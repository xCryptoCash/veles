 ...
 */
UniValue GetNetworkHashPS(int lookup, int height, int32_t nAlgo) {
...
UniValue getmininginfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"errors\": \"...\"            (string) DEPRECATED. Same as warnings. Only shown when fxtcd is started with -deprecatedrpc=getmininginfo\n"
            "}\n"
            "\nExamples:\n"
    ...
    obj.push_back(Pair("algo",             GetAlgoName(miningAlgo)));
...
UniValue estimatefee(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("estimatefee")) {
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "estimatefee is deprecated and will be fully removed in v0.17. "
            "To use estimatefee in v0.16, restart fxtcd with -deprecatedrpc=estimatefee.\n"
