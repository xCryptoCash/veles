...
static UniValue gethalvinginfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"smooth_halvings\": true|false,  (boolean) Equals to true if smooth halvings are enabled\n"
            "}\n"
            "\nExamples:\n"
    ...
    UniValue obj(UniValue::VOBJ);
    obj.pushKV("halvings_occured",   GetSubsidyHalvingParameters()->nHalvingInterval);
    obj.pushKV("halving_interval",   GetSubsidyHalvingParameters()->nHalvingCount);
    obj.pushKV("blocks_to_halving",  GetSubsidyHalvingParameters()->nBlocksToNextHalving);
    obj.pushKV("smooth_halvings",    sporkManager.IsSporkActive(SPORK_VELES_04_REWARD_UPGRADE_ALPHA_START));
    return obj;
}

static UniValue multialgostatus(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            "multialgostatus\n"
            ...
            "\nResult:\n"
            "{\n"
            "  \"xxxxxx\" : {                 (json object)  PoW algorithm algorithm name. Supported algorithms: sha256d, scrypt, lyra2z, x11, x16, nist5.\n"
            "    \"difficulty\": xxx.xxxxx,        (numeric) The current difficulty\n"
            "    \"networkhashps\": nnn,           (numeric) The network hashes per second\n"
            ...
            "}\n"
            "\nExamples:\n"
            + HelpExampleCli("multialgostatus", "")
            + HelpExampleRpc("multialgostatus", "")
        );
...
// VELES BEGIN
    ...
    { "mining",             "multialgostatus",        &multialgostatus,          {} },
// VELES END
