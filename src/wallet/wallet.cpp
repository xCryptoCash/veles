...
bool CWallet::GetBudgetSystemCollateralTX(CMutableTransaction& tx, uint256 hash, CAmount amount, bool fUseInstantSend)
{
    CWalletTx wtx;
...
// Dash
CAmount CWallet::GetRequiredFeeDash(unsigned int nTxBytes)
{
    return std::max(minTxFee.GetFee(nTxBytes), ::minRelayTxFee.GetFee(nTxBytes));
}

CAmount CWallet::GetMinimumFeeDash(unsigned int nTxBytes, unsigned int nConfirmTarget, const CTxMemPool& pool)
{
    ...
    nFeeNeeded = std::max(nFeeNeeded, GetRequiredFeeDash(nTxBytes));
